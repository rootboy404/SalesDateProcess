...
public class SaleService {
    ...
    public void addSale(String[] valueSale){
        if(valueSaleIsValid(valueSale)){
            ...
            saleList.add(Sale.builder().id(valueSale[1])
                    .itemList(itemList)
                    .name(valueSale[3])
                    .saleValue(new BigDecimal(itemList.stream().map(item -> item.getPrice().multiply(new BigDecimal(item.getQuantity()))).collect(Collectors.toList()).stream().mapToDouble(BigDecimal::doubleValue).sum()))
                    .build()
            );
        }
    }
    ...
    public String getIdMoreExpensiveSale(){
      return saleList.stream().max(Comparator.comparing(sale->sale.getSaleValue())).get().getId();
    }

    public String getWorstSeller(){
        return saleList.stream().collect(Collectors.groupingBy(Sale::getName,
